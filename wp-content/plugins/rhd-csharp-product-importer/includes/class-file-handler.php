...
class RHD_CSharp_File_Handler {
	...
	/**
	 * Import files for bundle products - now uses provided Product File Name (no ZIP creation)
	 */
	public function import_bundle_files( $product, $data ) {
		try {
			// Use the Product File Name directly as the downloadable file
			if ( !empty( $data['Product File Name'] ) ) {
				$this->import_protected_file( $product, $data['Product File Name'] );
			}

			// Image and sounds are handled by import_product_files after this call
			return true;
		} catch ( Exception $e ) {
